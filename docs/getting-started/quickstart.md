...
# 2. Auto-install 13 skills to ~/.gemini/skills/
chmod +x scripts/install_skills.sh
./scripts/install_skills.sh
```

The `install_skills.sh` script copies all 13 agent skill directories to `~/.gemini/skills/` and syncs required dependencies (`reportlab`, `pymupdf`, `pillow`).
